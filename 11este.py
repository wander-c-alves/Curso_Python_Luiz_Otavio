...
#         streamlit run 11este.py  esse comando deve ser executado no terminal
...
# 1. Configurações e Constantes (Sempre no topo)
TITULO_SISTEMA = "Vigilante Samambaia"

def main():
    """Função principal que renderiza o portal."""
    st.set_page_config(page_title=TITULO_SISTEMA, page_icon="📍")

    st.title(f"📍 {TITULO_SISTEMA}")
    st.subheader("Rua Vitor Meirelles")
    ...
    with tab2:
        st.header("Nova Ocorrência")
        tipo = st.selectbox("1. Tipo de Incidente", ["Segurança", "Infraestrutura", "Limpeza"])
        desc = st.text_area("2. Descrição do Problema")
...
# 2. Boilerplate profissional (Sempre no final)
